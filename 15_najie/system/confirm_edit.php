...
$login_id = $array_result[0];
//printf("登录的学号为%s",$login_id);
//printf("要修改的学号为%s",$id);
if($login_id!=$id){
	echo "<script> alert('你只能修修改自己的学号');
				window.history.back(); //返回上一页
          </script>";
}
//printf("%s",$name);
//写sql语句,判断该id是否已在studentinfo表中,如不在则insert into,如在则update
$lookForId = "select id from studentinfo where id = '{$id}'";
...
if(mysqli_query($Link,$sql_update)) {
      echo "<script> alert('修改成功');
                            window.location='personalinfo.php'; //跳转到首页
                 </script>";
     } else {
