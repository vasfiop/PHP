...
if (in_array(($action = get('action')), ['edit', 'delete'])) {
    ...
    $sex = post('sex');
    $user_right = post('user_right');
    $qq = post('qq');
    $email = post('email');

    if(intval($uid) === 1) $user_right = 1;
    ...
    //所以不考虑 SQL注入问题
    $update = $DB->query($sql=sprintf("UPDATE users SET nickname='%s' {$password}, sex=%d, user_right=%d ,qq=%d, email='%s' WHERE uid=%d",
        $nickname, $sex, $user_right, $qq, $email, $uid
    ));
...
if ($isLogin) {
    if (intval($uINFO['user_right']) === 1) {
        $sql = sprintf("SELECT uid+10000 AS uid,nickname,sex,email,qq,reg_ip,reg_time,user_right FROM users ORDER BY uid DESC LIMIT %d,%d",
            (intval($page) - 1) * 10, 10);
        $res = $DB->query($sql);
        ...
        //最大页码数量
        $max_page = ceil(($cccc = $DB->query("SELECT COUNT(*) AS c FROM `users`")->fetch_assoc()['c']) / 20);
    }
}
?>
<!DOCTYPE html>
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse navbar-right" id="example-navbar-collapse">
            <ul class="nav navbar-nav">
                <li class="active">
                    <a href="http://127.0.0.1/message-board-master/admin/index.php"><span class="glyphicon glyphicon-th-large" aria-hidden="true"></span>
                        用户管理</a>
                </li>
                <li>
                    <a href="msg.php">
        ...
        <table class="table table-hover">
            ...
            <?php foreach ($row as $v): ?>
                <tr data-uid="<?php echo $v['uid']; ?>">
                    ...
                    <td><?php echo ['否', '<span style="color: green;font-weight: bold">是</span>'][$v['user_right']]; ?></td>
                    ...
                    <td>
                        <a data-toggle="modal" data-target="#editUser"
                           ...
                           data-user_right="<?php echo $v['user_right']; ?>"
            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="form-group">
                        ...
                        <select id="user_right" class="form-control">
                            <option value="0" selected>普通用户</option>
                            <option value="1">管理员</option>
                        </select>
                        <span class="help-block">请注意，为了保证安全，你无法修改 UID 为 <code>10001</code> 的用户权限</span>
                    </div>
                    <div class="form-group">
